...
# --- Gráficos del Backtest Robusto (Bootstrap) ---

def plot_robust_sharpe_boxplot(details: pd.DataFrame, outpath: Path) -> None:
    """Boxplot de la distribución de Sharpe por perfil sobre múltiples senderos bootstrap."""
    profiles = [p for p in PROFILE_ORDER if p in details["profile_bucket"].unique()]
    series = [details.loc[details["profile_bucket"] == p, "sharpe_backtest"].dropna().values for p in profiles]
    labels = [p.replace("_", " ").title() for p in profiles]

    fig, ax = plt.subplots(figsize=(10, 6))
    bp = ax.boxplot(series, tick_labels=labels, showfliers=False, patch_artist=True, widths=0.6)
    for patch, color in zip(bp["boxes"], COLORS):
        patch.set_facecolor(color)
        patch.set_alpha(0.7)

    ax.axhline(y=0, color="#999999", linewidth=0.8, linestyle="-")
    ax.set_title("Backtest Robusto: Distribución de Sharpe Ratio por Perfil\n(250 senderos sintéticos — stationary bootstrap)", pad=15)
    ax.set_xlabel("Perfil de Riesgo")
...
def plot_robust_return_boxplot(details: pd.DataFrame, outpath: Path) -> None:
    """Boxplot de la distribución de retorno anualizado por perfil."""
    profiles = [p for p in PROFILE_ORDER if p in details["profile_bucket"].unique()]
    series = [details.loc[details["profile_bucket"] == p, "annual_return_backtest"].dropna().values * 100 for p in profiles]
    labels = [p.replace("_", " ").title() for p in profiles]
    ...
    ax.axhline(y=0, color="#999999", linewidth=0.8, linestyle="-")
    ax.set_title("Backtest Robusto: Distribución de Retorno Anualizado por Perfil\n(250 senderos sintéticos — stationary bootstrap)", pad=15)
    ax.set_xlabel("Perfil de Riesgo")
...
def plot_robust_drawdown_boxplot(details: pd.DataFrame, outpath: Path) -> None:
    """Boxplot de la distribución de drawdown máximo por perfil."""
    profiles = [p for p in PROFILE_ORDER if p in details["profile_bucket"].unique()]
    series = [details.loc[details["profile_bucket"] == p, "max_drawdown_backtest"].dropna().values * 100 for p in profiles]
    labels = [p.replace("_", " ").title() for p in profiles]
    ...
    ax.set_title("Backtest Robusto: Distribución de Drawdown Máximo por Perfil\n(250 senderos sintéticos — stationary bootstrap)", pad=15)
    ax.set_xlabel("Perfil de Riesgo")
...
def plot_robust_risk_return_scatter(summary: pd.DataFrame, outpath: Path) -> None:
    """Scatter de riesgo-retorno robusto con barras de error (IC 90%)."""
    profiles = [p for p in PROFILE_ORDER if p in summary["profile_bucket"].values]
    df = summary.set_index("profile_bucket").loc[profiles].reset_index()

    fig, ax = plt.subplots(figsize=(10, 6))

    xerr_low = df["annual_volatility_backtest_median"] - df["annual_volatility_backtest_p5"]
    xerr_high = df["annual_volatility_backtest_p95"] - df["annual_volatility_backtest_median"]
    yerr_low = df["annual_return_backtest_median"] - df["annual_return_backtest_p5"]
    yerr_high = df["annual_return_backtest_p95"] - df["annual_return_backtest_median"]

    ax.errorbar(
        df["annual_volatility_backtest_median"] * 100,
        df["annual_return_backtest_median"] * 100,
        xerr=[xerr_low.values * 100, xerr_high.values * 100],
        yerr=[yerr_low.values * 100, yerr_high.values * 100],
        fmt="o", markersize=10, color=COLORS[0], ecolor="#AAAAAA",
        elinewidth=1.5, capsize=5, capthick=1.5, zorder=3,
    )

    for _, row in df.iterrows():
        ax.annotate(
            str(row["profile_bucket"]).replace("_", " ").title(),
            (row["annual_volatility_backtest_median"] * 100, row["annual_return_backtest_median"] * 100),
            xytext=(8, 8), textcoords="offset points", fontsize=9, weight="bold",
        )

    ax.set_title("Backtest Robusto: Riesgo-Retorno Mediano por Perfil\n(barras de error = intervalo de confianza 90%)", pad=15)
    ax.set_xlabel("Volatilidad Anualizada Mediana (%)")
    ax.set_ylabel("Retorno Anualizado Mediano (%)")
    ax.margins(0.15)
    plt.tight_layout()
    plt.savefig(outpath, dpi=300, bbox_inches="tight")
    plt.close()
...
def main() -> None:
    ...
    # --- Gráficos del backtest robusto ---
    robust_details = _load_csv(output_dir, ["28_robust_backtest_scenario_details.csv"])
    robust_summary = _load_csv(output_dir, ["29_robust_backtest_summary.csv"])

    if robust_details is not None:
        plot_robust_sharpe_boxplot(robust_details, charts_dir / "17_robusto_sharpe_distribucion.png")
        plot_robust_return_boxplot(robust_details, charts_dir / "18_robusto_retorno_distribucion.png")
        plot_robust_drawdown_boxplot(robust_details, charts_dir / "19_robusto_drawdown_distribucion.png")

    if robust_summary is not None:
        plot_robust_risk_return_scatter(robust_summary, charts_dir / "20_robusto_riesgo_retorno_ic90.png")
