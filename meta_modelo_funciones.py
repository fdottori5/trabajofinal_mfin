...
# =========================
# 7b) BACKTEST ROBUSTO (Bootstrap)
# =========================

def robust_backtest_bootstrap(
    selected_profiles: pd.DataFrame,
    asset_returns_monthly: pd.DataFrame,
    rf_monthly: pd.Series,
    band: float = 0.03,
    initial_value: float = 100.0,
    n_boot_scenarios: int = 500,
    avg_block_size: float = 6.0,
    random_state: int = 42,
) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """
    Backtest robusto: corre el backtest con rebalanceo por bandas sobre
    múltiples senderos sintéticos generados con stationary bootstrap.

    Para cada perfil y cada escenario, se genera un sendero sintético de
    retornos mensuales (preservando dependencia temporal via bloques aleatorios),
    se ejecuta el backtest con los mismos pesos objetivo, y se registran las
    métricas de desempeño. El resultado es una distribución de outcomes por
    perfil, no un único número puntual.

    Returns
    -------
    scenario_details : pd.DataFrame
        Una fila por (perfil, escenario) con las métricas individuales.
    robust_summary : pd.DataFrame
        Estadísticos agregados por perfil: mediana, media, percentiles 5/25/75/95
        de retorno, volatilidad, Sharpe, drawdown, valor final, etc.
    """
    rng = np.random.default_rng(random_state)
    asset_columns = list(asset_returns_monthly.columns)
    n_obs = int(asset_returns_monthly.shape[0])

    if n_obs < 24:
        raise ValueError("Se requieren al menos 24 meses para el backtest robusto.")

    # Combinar retornos de activos y rf para muestrear de forma conjunta
    combined = pd.concat(
        [asset_returns_monthly, rf_monthly.rename("__rf__")],
        axis=1,
    ).dropna(how="any")
    n_obs = int(combined.shape[0])

    all_scenario_records: List[dict] = []

    for _, profile_row in selected_profiles.iterrows():
        profile_name = str(profile_row["profile_bucket"])
        target_weights = _extract_target_weights_from_row(profile_row, asset_columns)

        for scenario_id in range(1, n_boot_scenarios + 1):
            try:
                # Generar sendero sintético con stationary bootstrap
                boot_idx = stationary_bootstrap_indices(
                    n_obs=n_obs,
                    sample_length=n_obs,
                    avg_block_size=avg_block_size,
                ...
                scenario = combined.iloc[boot_idx].reset_index(drop=True)
                # Asignar índice temporal original para compatibilidad
                scenario.index = combined.index

                scenario_assets = scenario[asset_columns].copy()
                scenario_rf = scenario["__rf__"].copy()

                # Correr el backtest estándar sobre este sendero
                bt = simulate_band_rebalanced_portfolio(
                    asset_returns_monthly=scenario_assets,
                    target_weights=target_weights,
                    band=band,
                    initial_value=initial_value,
                )
                ...
                summary = summarize_backtest(
                    bt, rf_monthly=scenario_rf, profile_name=profile_name
                )
                ...
                all_scenario_records.append(record)

            except Exception:
                # Escenarios con problemas numéricos se descartan
                continue

    if not all_scenario_records:
        raise RuntimeError("No se completó ningún escenario de backtest robusto.")

    scenario_details = pd.DataFrame(all_scenario_records)

    # Agregar estadísticos por perfil
    metrics_to_agg = [
        "annual_return_backtest",
        "annual_volatility_backtest",
        "sharpe_backtest",
        "max_drawdown_backtest",
        "final_value_backtest",
        "n_rebalances",
        "annual_turnover_average",
    ]

    agg_records = []
    for profile_name, group in scenario_details.groupby("profile_bucket"):
        row = {"profile_bucket": profile_name, "n_scenarios_valid": len(group)}
        for metric in metrics_to_agg:
            ...
            row[f"{metric}_p5"] = float(vals.quantile(0.05))
            row[f"{metric}_p25"] = float(vals.quantile(0.25))
            row[f"{metric}_p75"] = float(vals.quantile(0.75))
            row[f"{metric}_p95"] = float(vals.quantile(0.95))
            # Probabilidad de outcome positivo (útil para Sharpe y retorno)
            if metric in ("sharpe_backtest", "annual_return_backtest"):
                row[f"{metric}_prob_positive"] = float(np.mean(vals.values > 0))
        agg_records.append(row)

    robust_summary = pd.DataFrame(agg_records)
    return scenario_details, robust_summary
...
def run_pipeline(
    ...
    # Backtest robusto: múltiples senderos sintéticos con stationary bootstrap
    robust_bt_details, robust_bt_summary = robust_backtest_bootstrap(
        selected_profiles=selected_profiles,
        asset_returns_monthly=asset_returns_monthly,
        rf_monthly=rf_monthly,
        band=rebalance_band,
        initial_value=initial_value,
        n_boot_scenarios=n_scenarios,  # reutiliza el mismo parámetro de escenarios
        avg_block_size=avg_block_size,
        random_state=random_state,
    )
    ...
    robust_bt_details.to_csv(output_dir / "28_robust_backtest_scenario_details.csv", index=False)
    robust_bt_summary.to_csv(output_dir / "29_robust_backtest_summary.csv", index=False)
    ...
    return {
        ...
        "robust_backtest_details": robust_bt_details,
        "robust_backtest_summary": robust_bt_summary,
    }
...
def main() -> None:
    ...
    if not outputs["robust_backtest_summary"].empty:
        print("\nResumen backtest ROBUSTO (stationary bootstrap, múltiples senderos):\n")
        robust_bt_cols = [c for c in outputs["robust_backtest_summary"].columns
                          if c.startswith("profile") or c.endswith(("_median", "_p5", "_p95", "_prob_positive"))
                          or c == "n_scenarios_valid"]
        print(outputs["robust_backtest_summary"][robust_bt_cols].to_string(index=False))
